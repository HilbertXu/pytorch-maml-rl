...
#     def __init__(self, gamma=0.95, device='cpu'):
#         self.gamma = gamma
#         self.device = device
...
#             self._lengths = len(self._observations_list)
#         return self._lengths

#     @property
#     def observation_shape(self):
#         if self._observation_shape is None:
#             self._observation_shape = self.observations.shape[1:]
#         return self._observation_shape

#     @property
#     def action_shape(self):
#         if self._action_shape is None:
#             self._action_shape = self.actions.shape[1:]
...
#             observation_shape = self._observations_list[0].shape
#             print (observation_shape)
#             print (len(self))
#             observations = np.zeros((len(self), ) + observation_shape,
#                                     dtype=np.float32)
#             # length = self.lengths[i]
#             np.stack(self._observations_list,
#                         axis=0,
#                         out=observations)
...
#             action_shape = self._actions_list[0].shape
#             actions = np.zeros((len(self), ) + action_shape,
#                                dtype=np.float32)
#             # length = self.lengths[i]
#             np.stack(self._actions_list, axis=0, out=actions)
...
#             rewards = np.zeros((len(self), ), dtype=np.float32)
#             # length = self.lengths[i]
#             np.stack(self._rewards_list, axis=0, out=rewards)
...
#             self._mask = torch.zeros((len(self), ),
#                                      dtype=torch.float32,
#                                      device=self.device)
#             # length = self.lengths[i]
#             self._mask.fill_(1.0)
#         return self._mask
...
#             return_ = torch.zeros((1,), dtype=torch.float32)
...
class BatchEpisodes(object):
    def __init__(self, batch_size, gamma=0.95, device='cpu'):
        ...
        self._speed_list = [[] for _ in range(batch_size)]
        self._task_list = [[] for _ in range(batch_size)]
    ...
    @property
    def task_list_return(self):
        return self._task_list

    @property
    def forward_speeds(self):
        return self._speed_list
    
    @property
    def rewards_list_return(self):
    ...
    def observations(self):
        if self._observations is None:
            ...
            # len(self): The number of trajectories of each task
    ...
    # def append(self, observations, actions, rewards):
    #     self._observations_list[0].append(observations.astype(np.float32))
    #     self._actions_list[0].append(actions.astype(np.float32))
    #     self._rewards_list[0].append(rewards.astype(np.float32))

    def append(self, observations, actions, rewards, infos, batch_ids):
        for observation, action, reward, info, batch_id in zip(
                observations, actions, rewards, infos, batch_ids):
            if batch_id is None:
                continue
            self._observations_list[batch_id].append(observation.astype(np.float32))
            self._actions_list[batch_id].append(action.astype(np.float32))
            self._rewards_list[batch_id].append(reward.astype(np.float32))
            self._speed_list[batch_id].append(info['forward_speed'].astype(np.float32))
            self._task_list[batch_id].append(info['task'].astype(np.float32))
    @property
    def logs(self):
        return self._logs
